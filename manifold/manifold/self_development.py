...
def _read_recent_sqlite_errors(max_errors: int = 15) -> str:
    import sqlite3
    db_path = get_path("manifold_activity.sqlite")
    if not os.path.exists(db_path):
        return "No activity DB found."
    try:
        conn = sqlite3.connect(db_path)
        conn.row_factory = sqlite3.Row
        cur = conn.cursor()
        rows = cur.execute("SELECT timestamp, component, message, data FROM activity_log WHERE type='error' ORDER BY id DESC LIMIT ?", (max_errors,)).fetchall()
        if not rows:
            return "No recent errors."
        errs = []
        for r in reversed(rows):
            errs.append(f"[{r['timestamp']}] {r['component']}: {r['message']} | Data: {r['data']}")
        return "\n".join(errs)
    except Exception as e:
        return f"Failed to read errors: {e}"
...
class SelfDevelopmentEngine:
    ...
    async def reflect_and_queue(self) -> None:
        ...
        recent_errors_excerpt = _read_recent_sqlite_errors(20)
        ...
        allowed_actions = (
            ...
            "4) append_orchestrator_directive: {type, directive, rationale, confidence}\n"
        )

        user_prompt = (
            "Recent Events (JSONL):\n"
            f"{json.dumps(events, indent=2)}\n\n"
            "Recent System Errors (SQLite Activity Log):\n"
            f"{recent_errors_excerpt}\n\n"
            "META.json Excerpt:\n"
            f"{meta_excerpt}\n\n"
