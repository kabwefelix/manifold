...
class ArchitectNode:
    ...
    async def _mutate_weights_file(self, new_instruction: str) -> bool:
        """
        Safely edits the cognitive_weights.json file to append a new instruction.
        Returns True if successful.
        """
        print(f"[ArchitectNode] Mutating cognitive weights: {new_instruction}")
        ...
        try:
            ...
            # We append the constraint to the Vector Observer prompt
            current_prompt = weights_data.get("system_prompts", {}).get("vector_observer", "")
            ...
            if new_instruction not in current_prompt:
                ...
                weights_data["system_prompts"]["vector_observer"] = updated_prompt
